...
/*================ 可配置参数 ====================*/
/* 1500us=0°，500us=-180°，2500us=180°；输入 -18000~18000（度×100） */
...
/* 输入 -18000~18000（度×100）→ 500~2500us，0→1500us */
static inline uint16_t raw_to_us(int16_t raw)
{
    int32_t v = (int32_t)raw;
    if (v < -18000) v = -18000;
    if (v > 18000) v = 18000;
    return (uint16_t)(SERVO_CENTER_US + (int32_t)v * 1000 / 18000);
}

/* 目标与当前（用于渐变） */
static uint16_t s_target_wrist_us   = 1500u;  /* 中位 */
static uint16_t s_target_gripper_us = 1500u;
static uint16_t s_current_wrist_us   = 1500u;
static uint16_t s_current_gripper_us = 1500u;
...
void Servo_Init(void)
{
    ...
    TIM_OCStructure.TIM_Pulse       = 1500u;
...
{
    s_target_wrist_us = raw_to_us(raw);
}

void Servo_SetGripper(int16_t raw)
{
    s_target_gripper_us = raw_to_us(raw);
}
