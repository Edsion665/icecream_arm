...
- 维度 `0..3`：J1~J4（电机关节角/角速度）
- 维度 `4`：手腕舵机关节角 `wrist_deg`
- 维度 `5`：夹爪状态 `grip_state`（推荐 `0=open, 1=close`）

## 4. 命令定义

### 4.1 `joints`（别名：`joint`、`axes`、`set_joints`）

| 字段 | 类型 | 必选 | 单位 | 约束 |
|---|---|---:|---|---|
| `axes_rel_deg` | array[number] | 是 | deg | 长度 4 / 5 / 6 |

语义：

- 长度 4：更新 J1~J4。
- 长度 5：更新 J1~J4 + `wrist_deg`（维度 4）。
- 长度 6：更新 J1~J4 + `wrist_deg` + `grip_state`（维度 5）。

错误：

- 长度非法：`invalid_length: axes_rel_deg 必须长度 4/5/6`。
- 第 6 维非法：`invalid_value: grip_state 必须为 0/1（或可阈值归一）`。

### 4.2 `joints_delta`（别名：`delta_joints`、`axes_delta`）

| 字段 | 类型 | 必选 | 单位 | 约束 |
|---|---|---:|---|---|
| `deltas_rel_deg` | array[number] | 是 | deg | 长度 4 / 5 / 6 |

语义：

- 长度 4：仅对 J1~J4 做增量。
- 长度 5：对 J1~J4 与 `wrist_deg` 做增量。
- 长度 6：对 J1~J4、`wrist_deg` 做增量，并更新 `grip_state`（离散值建议最终归一为 `0/1`）。

错误：

- 长度非法：`invalid_length: deltas_rel_deg 必须长度 4/5/6`。
- 第 6 维非法：`invalid_value: grip_state 必须为 0/1（或可阈值归一）`。

### 4.3 `claw`（别名：`wrist`、`gripper`）

可选输入组合（满足其一）：

1. `wrist_deg` + `grip`
2. `servo_deg=[wrist_deg, grip]`（长度 >= 2）
3. `wrist_deg` + `open_close`（`open|close` 或 `0|1`）

映射到 bridge->pi 6 维帧：

- `wrist_deg -> p_rel_deg[4]`
- `grip/open_close -> p_rel_deg[5]`（归一为 `grip_state`）

约束：

- `grip` 若使用连续值，bridge 侧应阈值归一为 `0/1` 再下发。
- 缺字段：`missing_field: claw 需要 wrist_deg + (grip/open_close)`。

### 4.4 其他命令

- `pose` / `pose_delta`：保持原语义。
...
| 数组长度非法 | 400 | `invalid_length: axes_rel_deg 必须长度 4/5/6` |
...
```json
{"cmd":"joints","axes_rel_deg":[0,10,-90,-70,15,1]}
{"cmd":"joints_delta","deltas_rel_deg":[0,0,0,0,5,0]}
